...
describe('classify', () => {
  ...
  it('maps knobs and joystick axes', () => { expect(classify({ type: 'cc', channel: 0, controller: 74, value: 64 }, DEFAULT_PROFILE, 1)).toMatchObject({ kind: 'knob', index: 4 }); expect(classify({ type: 'cc', channel: 0, controller: 1, value: 127 }, DEFAULT_PROFILE, 1)).toMatchObject({ kind: 'joyY', value: 1 }); expect(classify({ type: 'pitchBend', channel: 0, value: 8192 }, DEFAULT_PROFILE, 1)).toMatchObject({ kind: 'joyX' }) })
