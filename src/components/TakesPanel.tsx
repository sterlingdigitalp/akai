...
import { MAX_TAKES, useTakesStore } from '../state/takesStore'
...
export function TakesPanel() {
  ...
  const [message, setMessage] = useState(''); const [deletingId, setDeletingId] = useState<string | null>(null)
  const stopRef = useRef<(() => void) | null>(null)
  useEffect(() => { if (isRecording()) { setRecording(true); setElapsed(recordingElapsedMs()) } }, [])
  useEffect(() => {
    if (!recording) return
    const timer = window.setInterval(() => {
      setElapsed(recordingElapsedMs())
  ...
  const toggleRecord = () => {
    if (recording) { const take = stopRecording(`Take ${takes.length + 1}`); setRecording(false); setElapsed(null); if (take) { if (takes.length >= MAX_TAKES) setMessage('Saved. Your oldest take was removed to keep the newest 12.'); addTake(take) }; return }
    setMessage('')
    startRecording((reason) => {
      const current = useTakesStore.getState().takes
      const take = stopRecording(`Take ${current.length + 1}`)
      if (take) useTakesStore.getState().addTake(take)
      setRecording(false); setElapsed(null)
      const retention = current.length >= MAX_TAKES ? ' Your oldest take was removed to keep the newest 12.' : ''
      setMessage((reason === 'time-limit' ? 'Take saved automatically at the 3-minute limit.' : 'Take saved automatically at the 4,000-event limit.') + retention)
    }); setRecording(true); setElapsed(null)
  }
  ...
  return <>
    ...
    {takes.length === 0 ? <div className="empty-state takes-empty"><p className="lead">Hit record, play a few bars on the keys or pads, and your first take will land right here.</p></div> : <ul className="takes-list">{takes.map((take) => <li className="take-row" key={take.id}>
      ...
      {deletingId === take.id ? <span className="take-delete-confirm"><button className="text-button" onClick={() => { removeTake(take.id); setDeletingId(null) }}>Confirm delete</button><button className="text-button" onClick={() => setDeletingId(null)}>Cancel</button></span> : <button className="text-button take-delete" onClick={() => setDeletingId(take.id)}>Delete</button>}
    </li>)}</ul>}
    <DeviceView/>
  </>
}
