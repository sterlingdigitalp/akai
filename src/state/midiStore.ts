...
export const useMidiStore = create<MidiState>((set) => ({
  ...
  panic: () => set((state) => ({ heldKeys: new Set(), padFlash: null, wheels: { ...state.wheels, pitch: .5, mod: 0 } })),
  receive: (event) => set((state) => {
    if (event.kind === 'key') {
      const heldKeys = new Set(state.heldKeys)
      if (event.on) heldKeys.add(event.index)
      else heldKeys.delete(event.index)
      return { heldKeys }
    }
    if (event.kind === 'pad') return event.on === false
      ? (state.padFlash?.index === event.index ? { padFlash: null } : {})
      : { padFlash: { index: event.index, velocity: event.value, ts: event.ts } }
