...
export function Home() {
  ...
  return <section className="home"><div className="hero-copy"><div><p className="eyebrow">Your practice room</p><h1>Learn the instrument<br/>under your fingers.</h1><p className="lead">Four focused lessons take you from first touch to your own beat.</p></div><button className="button primary continue" onClick={() => openLesson(next.id)}><span>Continue</span><b>{next.title}</b><i>→</i></button></div><div className="section-heading"><div><p className="eyebrow">Lesson path</p><h2>Small steps. Real music.</h2></div><span>{Object.values(lessons).filter((l) => l.completedAt).length} of 4 complete</span></div><div className="lesson-grid">{LESSONS.map((lesson, index) => { const done = lessons[lesson.id]?.completedSteps.length ?? 0; const pct = done / lesson.steps.length; return <button className="lesson-card" key={lesson.id} style={{ '--delay': `${index * 60}ms` } as React.CSSProperties} onClick={() => openLesson(lesson.id)}><div className="lesson-card-top"><span className="lesson-number">{lesson.number}</span><span className="completion-ring" style={{ '--p': `${pct * 360}deg` } as React.CSSProperties}><i>{Math.round(pct * 100)}%</i></span></div><p className="eyebrow">{lesson.eyebrow}</p><h3>{lesson.title}</h3><p>{lesson.description}</p><footer><span>{lesson.steps.length - 1} moves</span><span>{lesson.minutes} min</span></footer></button> })}</div><PracticeStrip days={days}/></section>
}
