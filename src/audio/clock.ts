...
export function stepInterval(step: number, bpm: number, swing: number) {
  const duration = stepDurationSeconds(bpm)
  const clampedSwing = Math.max(0, Math.min(.6, swing))
  return duration + (step % 2 === 0 ? duration * clampedSwing : -duration * clampedSwing)
}
...
export class StepClock {
  ...
  private tick() { while (this.next < this.context.currentTime + .1) { this.callback(this.step % 16, this.next); this.next += stepInterval(this.step, this.bpm, this.swing); this.step++ } }
